...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        # Populate samples and labels using random number sample
        for i in range(n):
            random_numb = np.random.randint(0, n - 1)
            samples.append(XX[random_numb][:-1])
            labels.append(XX[random_numb][-1])
    ...
    def fitting(self):
        ...
        for i, row in enumerate(self.decision_trees):
            row.learn(self.bootstraps_datasets[i], self.bootstraps_labels[i])
        pass
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for i in self.decision_trees:
                    classified_records.append(i.classify(record))

                y = np.append(y, np.argmax(np.bincount(classified_records)))
                pass
            else:
