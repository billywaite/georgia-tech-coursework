...
def entropy(class_y):
    ...
    count = np.bincount(class_y)
    probs = count[np.nonzero(count)] / (y_len * 1.0)
    ...
    # Compute entropy
    ent = -np.sum(probs * np.log(probs)) / np.log(probs_len)
    return ent
...
def information_gain(previous_y, current_y):
    ...
    previous_entropy = 0
    for y in current_y:
        previous_entropy +=  entropy(y) * (len(y) * 1.0 / len(previous_y) * 1.0)

    info_gain = entropy(previous_y) - previous_entropy
